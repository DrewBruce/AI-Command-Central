...
pub async fn ask_provider(
    ...
    match config.mode.as_str() {
        ...
        "external" => Err(
            "External API providers are not wired yet. Use a local OpenAI-compatible endpoint for live runs."
                .to_string(),
        ),
        _ => Err("Live runs need a configured provider. Settings is currently in demo mode.".to_string()),
    }
}
...
async fn ask_local_provider_with_prompt(
    ...
    let content = body
        ...
        .ok_or_else(|| "Local provider returned no answer text.".to_string())?;
